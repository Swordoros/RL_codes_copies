...
class VPG_Agent:
    def __init__(self, env, policy_kwargs, baseline_kwargs=None, gamma=0.99):
        ...
        self.policy_net=self.build_network(output_size=self.action_n, output_actication=tf.nn.softmax, loss=keras.losses.categorical_crossentropy, **policy_kwargs)
        if baseline_kwargs:
            self.baseline_net=self.build_network(output_size=1, **baseline_kwargs)

    def build_network(self, hidden_sizes, output_size, activation=tf.nn.relu, output_actication=None, loss=keras.losses.mse , learning_rate=0.01):
...
agent=VPG_Agent(env, policy_kwargs=policy_kwargs)
'''
